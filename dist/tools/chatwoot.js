...
const client = () => createClient(config.chatwoot.url, {
...
export async function handleChatwootTool(name, args) {
    ...
    switch (name) {
        case 'chatwoot_list_conversations': {
            ...
            if (args.inbox_id)
                params.inbox_id = args.inbox_id;
            if (args.assignee_type)
                params.assignee_type = args.assignee_type;
        ...
        case 'chatwoot_send_message': {
            ...
            const res = await safeRequest(() => http.post(`${base}/conversations/${args.conversation_id}/messages`, payload).then(r => r.data));
            return toText(res);
        }
        case 'chatwoot_assign_conversation': {
            ...
            const res = await safeRequest(() => http.patch(`${base}/conversations/${args.conversation_id}/assignments`, payload).then(r => r.data));
            return toText(res);
        }
        ...
        case 'chatwoot_get_reports': {
            ...
            const res = await safeRequest(() => http.get(`${base}/reports/agents/summary`, { params }).then(r => r.data));
            return toText(res);
        }
        default:
