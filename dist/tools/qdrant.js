...
export async function handleQdrantTool(name, args) {
    const http = client();
    switch (name) {
        ...
        case 'qdrant_collection_info': {
            const res = await safeRequest(() => http.get(`/collections/${args.collection}`).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_create_collection': {
            ...
            const res = await safeRequest(() => http.put(`/collections/${args.collection}`, payload).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_delete_collection': {
            const res = await safeRequest(() => http.delete(`/collections/${args.collection}`).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_upsert_points': {
            const payload = { points: args.points };
            const res = await safeRequest(() => http.put(`/collections/${args.collection}/points`, payload).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_search': {
            const payload = {
            ...
            const res = await safeRequest(() => http.post(`/collections/${args.collection}/points/search`, payload).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_get_points': {
            ...
            const res = await safeRequest(() => http.post(`/collections/${args.collection}/points`, payload).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_delete_points': {
            const selector = {};
            if (args.ids)
                selector.points = args.ids;
            else if (args.filter)
                selector.filter = args.filter;
            const res = await safeRequest(() => http.post(`/collections/${args.collection}/points/delete`, selector).then(r => r.data));
            return toText(res);
        }
        case 'qdrant_create_payload_index': {
            const payload = { field_name: args.field_name, field_schema: args.field_schema };
            const res = await safeRequest(() => http.put(`/collections/${args.collection}/index`, payload).then(r => r.data));
            return toText(res);
        }
        default:
