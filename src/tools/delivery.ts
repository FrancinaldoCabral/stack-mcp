...
function findContextInfo(obj: unknown): { stanzaId?: string } | null {
  if (!obj || typeof obj !== 'object') return null;
  const o = obj as Record<string, unknown>;
  if ('stanzaId' in o) return o as { stanzaId?: string };
  if ('contextInfo' in o && o.contextInfo) return findContextInfo(o.contextInfo);
    ...
    case 'delivery_deliverer_context': {
      ...
      if (args.instance && args.waExternalId) {
        try {
          ...
          const records: unknown[] = evoRes.data?.messages?.records
          ...
          if (ctxInfo?.stanzaId) replyId = String(ctxInfo.stanzaId);
        } catch (e) {
