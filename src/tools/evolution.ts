...
export const evolutionTools: Tool[] = [
  ...
  },
  {
    ...
    description:
      'Envia múltiplas mensagens em sequência com delays entre elas, simulando digitação humana natural. Ideal para respostas de agentes — divida em pedaços curtos de texto, emojis isolados, stickers e áudios curtos.',
    inputSchema: {
      type: 'object',
...
export async function handleEvolutionTool(name: string, args: Args): Promise<string> {
  ...
  switch (name) {
    ...
    case 'evolution_send_list': {
      ...
      const payload: Record<string, unknown> = { number, title, description, buttonText, footerText, values, delay: delay ?? 0 };
      const res = await safeRequest(() =>
    ...
    case 'evolution_send_buttons': {
      ...
      const payload: Record<string, unknown> = { number, title, description, footer, buttons, delay: delay ?? 0 };
      const res = await safeRequest(() =>
        http.post(`/message/sendButtons/${instanceName}`, payload).then(r => r.data)
      );
      return toText(res);
    }
    ...
    case 'evolution_send_presence': {
      ...
      const res = await safeRequest(() =>
        http.post(`/chat/sendPresence/${instanceName}`, payload).then(r => r.data)
      );
      return toText(res);
    }
    default:
